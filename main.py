...
def process_transcript_result(message,bot_obj,corrected_text):
    ...
    if len(corrected_text)>4000:
        if user_mode=="file":
            ...
            sent=bot_obj.send_document(message.chat.id,f,reply_to_message_id=reply_to_id)
            ...
            for idx,chunk in enumerate(chunks):
                if idx==0: last_sent=bot_obj.send_message(message.chat.id,chunk,reply_to_message_id=reply_to_id)
            ...
            sent_message_id=last_sent.message_id if last_sent else None
    else:
        sent_msg=bot_obj.send_message(message.chat.id,corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=reply_to_id)
    ...
    if sent_message_id:
        try:
            user_transcriptions.setdefault(uid_key,{})[sent_message_id]=corrected_text
            threading.Thread(target=delete_transcription_later,args=(uid_key,sent_message_id),daemon=True).start()
        except: pass

def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename,file_url=None):
    ...
    try:
        ...
        if source_url is None: 
            tf,source_url=telegram_file_info_and_url(bot_token,file_id)
            
        try:
            text,used_service=transcribe_via_selected_service(source_url,lang)
        except Exception as e:
            ...
            if "ffmpeg" in error_msg.lower() and not FFMPEG_BINARY:
                ...
                bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible or link is invalid. Please send a different file.",reply_to_message_id=message.message_id)
            else:
        ...
        process_transcript_result(message,bot_obj,corrected_text)
...
def worker_thread():
    while True:
        try:
            ...
            if item:
                message,bot_obj,bot_token,file_id,file_size,filename,file_url=item
                ...
                # process_media_file handles both telegram file_id and external file_url
                process_media_file(message,bot_obj,bot_token,file_id,file_size,filename,file_url)
            else:
...
def handle_media_common(message,bot_obj,bot_token):
    ...
    with memory_lock:
        ...
        PENDING_QUEUE.append((message,bot_obj,bot_token,file_id,file_size,filename,None))

def handle_link_common(message,bot_obj):
    if not bot_obj: return
    update_user_activity(message.from_user.id)
    if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
        send_subscription_message(message.chat.id,bot_obj); return
    ...
    with memory_lock:
        if len(PENDING_QUEUE)>=MAX_PENDING_QUEUE:
            bot_obj.send_message(message.chat.id,"⚠️ Server busy. Try again later.",reply_to_message_id=message.message_id); return
        # file_id, file_size, and filename are None for external links
        PENDING_QUEUE.append((message,bot_obj,BOT_TOKEN,None,None,None,file_url))
...
def register_handlers(bot_obj,bot_token):
    ...
    def start_select_lang_callback(call):
        try:
            ...
            welcome_text="👋 Salaam!    \n• Send me\n• voice message\n• audio file\n• video\n• or a direct file link (e.g., file.io, limewire, or any direct URL) \n• to transcribe for free"
    ...
    def handle_help(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            text="Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video (up to 20MB) or a direct file link (e.g., file.io, limewire) and I will transcribe it Need help? Contact: @lakigithub"
    ...
    def handle_text_messages(message):
        try:
            ...
            # Check for specific links (file.io, limewire) or any general http/https link
            link_pattern = r'https?:\/\/(?:www\.)?(?:file\.io\/[a-zA-Z0-9]+|limewire\.com\/d\/[a-zA-Z0-9#]+|[a-zA-Z0-9\-\.]+\.[a-z]{2,6}(?:\/[^\s]*)?)'
            
            if re.match(link_pattern,text):
                handle_link_common(message,bot_obj); return
    ...
    def clean_up_callback(call):
        try:
            ...
            # Re-use process_transcript_result for cleaned text
            cleaned_message = call.message
            cleaned_message.message_id = call.message.message_id # Use original message for reply_to if possible
            process_transcript_result(cleaned_message, bot_obj, cleaned)
