...
ASSEMBLYAI_SUPPORTED_LANGUAGES=set(["en","es","fr","de","it","pt"])
...
def transcribe_with_assemblyai(input_file_path,language_code):
    ...
    try:
        with open(input_file_path,"rb") as f:
            upload_response=requests.post(upload_url,headers=headers,data=f,timeout=REQUEST_TIMEOUT_GEMINI)
        upload_response.raise_for_status()
        audio_url=upload_response.json().get("upload_url")
    except Exception as e: raise RuntimeError("AssemblyAI upload failed: "+str(e))
    config={"audio_url":audio_url,"auto_chapters":False,"auto_highlights":False,"word_boost":False,"punctuate":True,"format_text":True}
    if language_code:
        config["language_code"]=language_code
        if language_code!="en": config["language_detection"]=False
    transcript_url="https://api.assemblyai.com/v2/transcript"
    try:
        submit_response=requests.post(transcript_url,headers=headers,json=config,timeout=REQUEST_TIMEOUT_GEMINI)
        submit_response.raise_for_status()
        transcript_id=submit_response.json().get("id")
    except Exception as e: raise RuntimeError("AssemblyAI submit failed: "+str(e))
    polling_endpoint=f"{transcript_url}/{transcript_id}"
    while True:
        try:
            polling_response=requests.get(polling_endpoint,headers=headers,timeout=REQUEST_TIMEOUT_GEMINI)
            polling_response.raise_for_status()
            result=polling_response.json()
            status=result.get("status")
            if status=="completed": return result.get("text")
            elif status=="error": raise RuntimeError("AssemblyAI transcription error: "+result.get("error"))
            elif status=="queued" or status=="processing": time.sleep(5)
            else: raise RuntimeError("AssemblyAI unknown status: "+status)
        except requests.exceptions.Timeout: time.sleep(5)
        except Exception as e: raise RuntimeError("AssemblyAI polling failed: "+str(e))

def transcribe_via_selected_service(input_path,lang_code):
    use_assemblyai=ASSEMBLYAI_API_KEY and lang_code in ASSEMBLYAI_SUPPORTED_LANGUAGES
    if use_assemblyai:
        try:
            ...
            logging.warning(f"AssemblyAI failed for language {lang_code}, falling back to speech_recognition: "+str(e))
    try:
...
def process_media_file(message,bot_obj,bot_token,bot_index,file_id,file_size,filename):
    ...
    try:
        ...
        try:
            ...
            elif "AssemblyAI" in error_msg and used_service!="assemblyai":
                bot_obj.send_message(message.chat.id,f"Error during transcription (AssemblyAI): {error_msg}",reply_to_message_id=message.message_id)
            else:
