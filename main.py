...
from pyrogram.errors import FileTooBig, FloodWait, RPCError

logging.basicConfig(level=logging.INFO,format='%(asctime)s - %(levelname)s - %(message)s')

env=os.environ

# Increased max bytes for media that will be handled by Pyrogram for large files (up to 2GB)
TELEGRAM_MAX_BYTES=int(env.get("TELEGRAM_MAX_BYTES",str(2048*1024*1024))) # 2GB
...
# Pyrogram required environment variables
API_ID=env.get("API_ID","29169428")
API_HASH=env.get("API_HASH","55742b16a85aac494c7944568b5507e5")

if not BOT_TOKEN: logging.error("BOT_TOKEN is not set. Bot will not function.")
if not API_ID or not API_HASH: logging.warning("API_ID or API_HASH not set. Pyrogram large file download will not work.")
...
# Initialize Pyrogram Client (must be done outside of the Flask app context for threading)
pyro_client=None
if BOT_TOKEN and API_ID and API_HASH:
    try:
        pyro_client = Client(
            ...
            api_hash=API_HASH,
            bot_token=BOT_TOKEN,
            workers=10 # Use multiple workers for concurrent downloads
        )
        pyro_client.start()
...
def download_file_pyrogram(message,bot_obj,pyro_client):
    try:
        temp_dir = tempfile.gettempdir()
        file_path = pyro_client.download_media(message, file_name=os.path.join(temp_dir, "%f"))
        return file_path
    except FileTooBig:
        raise RuntimeError(f"File is too big, even for Pyrogram (max 2GB).")
    except FloodWait as e:
        raise RuntimeError(f"Pyrogram FloodWait: wait {e.value} seconds.")
    except RPCError as e:
        raise RuntimeError(f"Pyrogram RPC Error: {e}")
    except Exception as e:
        raise RuntimeError(f"Pyrogram download failed: {e}")

def download_file_requests(bot_token,file_id,filename):
...
def transcribe_file_with_assemblyai(input_file_path,language_code):
...
def transcribe_via_selected_service(input_path,lang_code):
    try:
        text,service=transcribe_file_with_assemblyai(input_path,lang_code),"assemblyai"
...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename,use_pyrogram=False):
    ...
    try:
        if use_pyrogram:
            if not pyro_client: raise RuntimeError("Pyrogram client is not initialized for large file download.")
            logging.info(f"Downloading file {file_id} with Pyrogram for user {uid}")
            tmpf_name = download_file_pyrogram(message, bot_obj, pyro_client)
        else:
            logging.info(f"Downloading file {file_id} with requests (small file) for user {uid}")
            tmpf_name = download_file_requests(bot_token, file_id, filename)
        ...
        error_msg=str(e); logging.exception("Error in process_media_file")
        if "Pyrogram FloodWait" in error_msg:
             bot_obj.send_message(message.chat.id,f"⚠️ Pyrogram temporarily unavailable. Please try again later. Error: {error_msg}",reply_to_message_id=message.message_id)
        else:
             bot_obj.send_message(message.chat.id,f"An unexpected error occurred: {error_msg}",reply_to_message_id=message.message_id)
    finally:
        if tmpf_name:
...
def worker_thread():
    while True:
        try:
            ...
            if item:
                message,bot_obj,bot_token,file_id,file_size,filename,use_pyrogram=item
                ...
                process_media_file(message,bot_obj,bot_token,file_id,file_size,filename,use_pyrogram)
            else:
...
# Get the default maximum file size allowed by the Telegram Bot API (20MB)
TELEGRAM_BOTAPI_MAX_BYTES = 20 * 1024 * 1024
...
def handle_media_common(message,bot_obj,bot_token):
    ...
    file_id=file_size=filename=None; media_message=None
    
    if message.voice:
        media_message=message; file_id=message.voice.file_id; file_size=message.voice.file_size; filename="voice.ogg"
    elif message.audio:
        media_message=message; file_id=message.audio.file_id; file_size=message.audio.file_size; filename=getattr(message.audio,"file_name","audio")
    elif message.video:
        media_message=message; file_id=message.video.file_id; file_size=message.video.file_size; filename=getattr(message.video,"file_name","video.mp4")
        ...
        if (mime and ("audio" in mime or "video" in mime)) or ext in ALLOWED_EXTENSIONS:
            media_message=message; file_id=message.document.file_id; file_size=message.document.file_size
        else:
            bot_obj.send_message(message.chat.id,"Sorry, I can only transcribe audio or video files."); return

    if not file_id: return # Should not happen if a media type is matched

    # Determine if we need to use Pyrogram
    use_pyrogram=False
    if pyro_client and file_size and file_size > TELEGRAM_BOTAPI_MAX_BYTES:
        use_pyrogram=True
        
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        max_display_gb=TELEGRAM_MAX_BYTES//(1024*1024*1024)
        bot_obj.send_message(message.chat.id,f"Just Send me a file less than {max_display_gb}GB (2GB) 😎",reply_to_message_id=message.message_id); return
    
    if use_pyrogram and not pyro_client:
        bot_obj.send_message(message.chat.id,"File is too big for standard download. Please set API_ID and API_HASH for large file support (up to 2GB).",reply_to_message_id=message.message_id); return
    ...
    with memory_lock:
        ...
        PENDING_QUEUE.append((media_message,bot_obj,bot_token,file_id,file_size,filename,use_pyrogram))
...
def register_handlers(bot_obj,bot_token):
    ...
    def handle_help(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            text="Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video (up to 2GB with Pyrogram) and I will transcribe it Need help? Contact: @lakigithub"
...
if __name__ == "__main__":
    if not BOT_TOKEN:
        ...
        # Stop Pyrogram client when the Flask app shuts down (or main script exits)
        if pyro_client:
            try:
                pyro_client.stop()
