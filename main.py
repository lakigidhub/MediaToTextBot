...
def telegram_file_info_and_url(bot_token,file_id):
    ...
    # Tani waxay noqon doontaa URL-ka Telegram-ka. Haddii AssemblyAI uu si toos ah uga soo dejisan karo, ma u baahnid inaad adigu soo dejiso.
    return f"https://api.telegram.org/file/bot{bot_token}/{file_path}" if file_path else None

def transcribe_file_with_assemblyai(input_data,language_code,is_url=True):
    ...
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            ...
            if not is_url: # Haddii ay tahay faylka maxalliga ah (local file) (ilaa 20MB)
            ...
            else: # Haddii ay tahay URL (wuxuu la shaqeyn karaa faylal waaweyn)
...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename):
    ...
    try:
        # Xalka 1: Haddii faylku ka yar yahay xadka, soo dejiso oo u dir AssemblyAI upload ahaan
        if file_size and file_size <= TELEGRAM_MAX_BYTES:
            file_url=telegram_file_info_and_url(bot_token,file_id)
            if not file_url: raise RuntimeError("Could not get file URL.")
            ...
            text,used_service=transcribe_via_selected_service(tmpf_name,lang,is_url=False)
            
        # Xalka 2: Haddii faylku weyn yahay, u dir URL-ka AssemblyAI si uu si toos ah uga soo dejisto Telegram
        else: # file_size > TELEGRAM_MAX_BYTES ama file_size waa None (marka hore ha u ekaanina file_size > TELEGRAM_MAX_BYTES si aad u kordhiso)
            file_url=telegram_file_info_and_url(bot_token,file_id)
            if not file_url: raise RuntimeError("Could not get file URL.")
...
def handle_media_common(message,bot_obj,bot_token):
    ...
    # Xadka cabirka Telegram_MAX_BYTES hadda ma noqon doono mid mamnuucaya dirista faylka.
    # Waan iska ilaalinaynaa inaan soo dejinno faylal waaweyn haddii aan dirayno URL.
    # Weli waxaad u baahan tahay inaad hubiso in faylka uu u hoggaansamo xadka Telegram (ilaa 2GB), taas oo aanu halkan ku hubinaynin.
    # Si kastaba ha ahaatee, haddii file_size uu jiro oo uu ka weyn yahay 20MB, kaliya waanu diraynaa URL-ka.
