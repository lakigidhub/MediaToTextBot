import os,logging,requests,telebot,json,threading,time,io,subprocess,tempfile
...
ASSEMBLYAI_API_KEY=env.get("ASSEMBLYAI_API_KEY","") # New required key
...
# Set AssemblyAI API Key if available
if ASSEMBLYAI_API_KEY:
    aai.settings.api_key=ASSEMBLYAI_API_KEY
else:
    logging.error("ASSEMBLYAI_API_KEY is not set. Transcription will fail.")
...
# AssemblyAI supported languages (subset for clarity, full list is larger)
ASSEMBLYAI_SUPPORTED_LANGS={'en','ar','es','fr','ru','de','hi','fa','id','uk','az','it','tr','bg','sr','ur','th','vi','ja','ko','zh','nl','sv','no','he','da','am','fi','bn','sw','om','ne','pl','el','cs','is','lt','lv','hr','bs','hu','ro','so','ms','uz','tl','pt'}
LANG_OPTIONS=[(label,code) for label,code in [(p.split(":",1)[0].strip(),p.split(":",1)[1].strip()) for p in _LANG_RAW.split(",")] if code in ASSEMBLYAI_SUPPORTED_LANGS]
...
def get_stt_user_lang(user_id):
    ...
    lang=ud.get("stt_language","en") if ud else "en"
    return lang if lang in ASSEMBLYAI_SUPPORTED_LANGS else "en"

def set_stt_user_lang(user_id,lang_code):
    if lang_code in ASSEMBLYAI_SUPPORTED_LANGS:
...
def is_transcoding_like_error(msg):
    # AssemblyAI doesn't use local transcoding, this is mostly for general file issues
    if not msg: return False
    m=msg.lower()
    checks=["unsupported media type","could not decode"]
...
def normalize_text_offline(text):
    # AssemblyAI provides good normalization, but this is a final cleanup
    return text.strip() if text else ""

def extract_key_points_offline(text,max_points=6):
    if not text: return ""
    # Simple extractive summarization fallback if Gemini fails or is not configured
    sentences=[s.strip() for s in text.split(". ") if s.strip()] # Simple splitting for fallback
...
def transcribe_with_assemblyai(input_file_path,language_code):
    if not ASSEMBLYAI_API_KEY: raise RuntimeError("AssemblyAI API key not set")
    # AssemblyAI Transcription Configuration
    config=aai.TranscriptionConfig(language_code=language_code)
    try:
        # 1. Upload the file
        uploader=aai.Uploader()
        upload_url=uploader.upload(input_file_path)
...
def process_media_file(message,bot_obj,bot_token,bot_index,file_id,file_size,filename):
    ...
    try:
        ...
        try:
            ...
            if "AssemblyAI API key not set" in error_msg:
                 bot_obj.send_message(message.chat.id,"⚠️ Server error: AssemblyAI API key is missing. Notify the admin.",reply_to_message_id=message.message_id)
            elif is_transcoding_like_error(error_msg):
                bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible or an unsupported format. Please send a different file.",reply_to_message_id=message.message_id)
            else:
        ...
        if len(corrected_text)>4000:
            ...
            sent_msg=bot_obj.send_message(message.chat.id,corrected_text or f"⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed: {CODE_TO_LABEL.get(lang)}.",reply_to_message_id=message.message_id)
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    if not ASSEMBLYAI_API_KEY:
        bot_obj.send_message(message.chat.id,"⚠️ Server error: AssemblyAI API key is missing. Notify the admin.",reply_to_message_id=message.message_id); return
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def start_handler(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            if not ASSEMBLYAI_API_KEY:
                bot_obj.send_message(message.chat.id,"⚠️ Server error: AssemblyAI API key is missing. Transcription is unavailable.",reply_to_message_id=message.message_id); return
    ...
    def handle_help(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            text="Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video (up to 20MB) and I will transcribe it using AssemblyAI. Need help? Contact: @lakigithub"
