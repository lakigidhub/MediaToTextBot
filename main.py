...
def transcribe_file_with_assemblyai(audio_source,language_code):
    ...
    last_exception=None
    for api_key_raw in ASSEMBLYAI_API_KEYS:
        api_key=api_key_raw.strip() # Strip again just in case
        try:
...
def send_transcription_result(bot_obj,chat_id,message_id,corrected_text):
    uid_key=str(chat_id); user_mode=get_user_send_mode(uid_key); last_sent=None
    if len(corrected_text)>4000:
        if user_mode=="file":
            f=io.BytesIO(corrected_text.encode("utf-8")); f.name="Transcript.txt"
            last_sent=bot_obj.send_document(chat_id,f,reply_to_message_id=message_id)
        else:
            chunks=split_text_into_chunks(corrected_text,limit=4096)
            for idx,chunk in enumerate(chunks):
                if idx==0: last_sent=bot_obj.send_message(chat_id,chunk,reply_to_message_id=message_id)
                else: last_sent=bot_obj.send_message(chat_id,chunk)
    else:
        last_sent=bot_obj.send_message(chat_id,corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=message_id)
...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename):
    ...
    try:
        ...
        send_transcription_result(bot_obj,message.chat.id,message.message_id,corrected_text)
...
def process_link_file(message,bot_obj,audio_link):
    uid=str(message.from_user.id); chatid=str(message.chat.id)
    lang=get_stt_user_lang(uid)
    processing_msg=bot_obj.send_message(message.chat.id,"🔄 Processing...",reply_to_message_id=message.message_id)
    processing_msg_id=processing_msg.message_id
    ...
    try:
        try:
            text,used_service=transcribe_via_selected_service(audio_link,lang)
        except Exception as e:
            error_msg=str(e); logging.exception("Error during transcription from link")
            bot_obj.send_message(message.chat.id,f"Error during transcription from link: {error_msg}",reply_to_message_id=message.message_id)
            return
        corrected_text=normalize_text_offline(text)
        send_transcription_result(bot_obj,message.chat.id,message.message_id,corrected_text)
        increment_processing_count(uid,"stt")
...
def register_handlers(bot_obj,bot_token):
    ...
    def start_select_lang_callback(call):
        try:
            ...
            welcome_text="👋 Salaam!    \n• Send me\n• voice message\n• audio file\n• video\n• or an audio/video link (e.g., limewire.com/d/...) to transcribe for free"
    ...
    def handle_help(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            text="Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video (up to 20MB) or an audio/video link and I will transcribe it. Need help? Contact: @lakigithub"
    ...
    def handle_text_messages(message):
        try:
            ...
            # Check for limewire link pattern or any similar audio/video direct link
            limewire_pattern=re.compile(r"https?:\/\/(www\.)?limewire\.com\/d\/[a-zA-Z0-9#\-\._]+")
            url_match=limewire_pattern.search(text)
            
            if url_match:
                audio_link=url_match.group(0)
                # Process the link in a worker thread to prevent blocking
                t=threading.Thread(target=process_link_file,args=(message,bot_obj,audio_link))
                t.start()
            else:
