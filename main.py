...
BOT_TOKEN=env.get("BOT_TOKEN","") or (BOT_TOKENS[0] if BOT_TOKENS else "")
ASSEMBLYAI_API_KEY=env.get("ASSEMBLYAI_API_KEY","")
ASSEMBLYAI_API_KEYS_RAW=env.get("ASSEMBLYAI_API_KEYS","")
if ASSEMBLYAI_API_KEYS_RAW:
    ASSEMBLYAI_API_KEYS=[k.strip() for k in ASSEMBLYAI_API_KEYS_RAW.split(",") if k.strip()]
elif ASSEMBLYAI_API_KEY:
    ASSEMBLYAI_API_KEYS=[ASSEMBLYAI_API_KEY]
else:
    ASSEMBLYAI_API_KEYS=[]
...
app=Flask(__name__)
if not BOT_TOKEN:
    logging.error("No BOT_TOKEN provided in environment")
botsingle=telebot.TeleBot(BOT_TOKEN,threaded=True,parse_mode='HTML')

_LANG_RAW="🇬🇧 English:en,🇸🇦 العربية:ar,🇪🇸 Español:es,🇫🇷 Français:fr,🇷🇺 Русский:ru,🇩🇪 Deutsch:de,🇮🇳 हिन्दी:hi,🇮🇷 فارسی:fa,🇮🇩 Indonesia:id,🇺🇦 Українська:uk,🇦🇿 Azərbaycan:az,🇮🇹 Italiano:it,🇹🇷 Türkçe:tr,🇧🇬 Български:bg,🇷🇸 Srpski:sr,🇵🇰 اردو:ur,🇹🇭 ไทย:th,🇻🇳 Tiếng Việt:vi,🇯🇵 日本語:ja,🇰🇷 한국어:ko,🇨🇳 中文:zh,🇳🇱 Nederlands:nl,🇸🇪 Svenska:sv,🇳🇴 Norsk:no,🇮🇱 עבריთ:he,🇩🇰 Dansk:da,🇪🇹 አማርኛ:am,🇫🇮 Suomi:fi,🇧🇩 বাংলা:bn,🇰🇪 Kiswahili:sw,🇪🇹 Oromoo:om,🇳🇵 नेपाली:ne,🇵🇱 Polski:pl,🇬🇷 Ελληνικά:el,🇨🇿 Čeština:cs,🇮🇸 Íslenska:is,🇱🇹 Lietuvių:lt,🇱🇻 Latviešu:lv,🇭🇷 Hrvatski:hr,🇷🇸 Bosanski:bs,🇭🇺 Magyar:hu,🇷🇴 Română:ro,🇸🇴 Somali:so,🇲🇾 Melayu:ms,🇺🇿 O'zbekcha:uz,🇵🇭 Tagalog:tl,🇵🇹 Português:pt"
...
def transcribe_file_with_assemblyai(input_file_path,language_code):
    if not ASSEMBLYAI_API_KEYS: raise RuntimeError("No ASSEMBLYAI API keys configured")
    last_exc=None
    for key in ASSEMBLYAI_API_KEYS:
        try:
            ...
            last_exc=e
            logging.exception("AssemblyAI key failed, trying next key if available")
            continue
    raise RuntimeError("All AssemblyAI keys failed: "+str(last_exc))

def transcribe_via_selected_service(input_path,lang_code):
    try:
        text=transcribe_file_with_assemblyai(input_path,lang_code)
...
register_handlers(botsingle,BOT_TOKEN,0)

@app.route("/",methods=["GET","POST","HEAD"])
def webhook_root():
    if request.method in ("GET","HEAD"):
        now_iso=datetime.utcnow().isoformat()+"Z"
        return jsonify({"status":"ok","time":now_iso,"bot_index":0}),200
    if request.method=="POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            try:
                ...
                botsingle.process_new_updates([update])
...
def set_webhook_route():
    try:
        url=WEBHOOK_BASE.rstrip("/")+f"/"
        botsingle.delete_webhook(); time.sleep(0.2)
        botsingle.set_webhook(url=url)
        return jsonify({"index":0,"url":url,"status":"ok"}),200
    except Exception as e:
...
def delete_webhook_route():
    try: botsingle.delete_webhook(); return jsonify({"index":0,"status":"deleted"}),200
    except Exception as e:
        logging.error(f"Failed to delete webhook: {e}"); return jsonify({"error":str(e)}),500
