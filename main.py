...
def transcribe_file_with_assemblyai(input_file_path_or_url,language_code,is_url=False):
    ...
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            ...
            if is_url:
                audio_url=input_file_path_or_url # Isticmaal URL-ka si toos ah
            else:
...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename):
    ...
    try:
        ...
        text = None # Ku dar isbeddelkan si aad u hubiso helitaanka text-ka

        if is_large_file:
            # Faylalka waaweyn: U dir URL-ka AssemblyAI
            try:
            ...
            # Faylalka yaryar: Soo degso ka hor inta aadan u dirin AssemblyAI
        ...
        # Haddii uusan jirin wax qalad ah oo keenay inuu return gareeyo:
        corrected_text=normalize_text_offline(text)
        uid_key=str(message.chat.id); user_mode=get_user_send_mode(uid_key)
        
        # Qaybta loo diro natiijada. Waa in ay dhacdaa marka transcription-ka uu guulaysto
        ...
        # Hubi inaad joojiso animation-ka ka hor inta aan la tirtirin fariinta
        stop["stop"]=True; animation_thread.join()
        
        # Had iyo jeer tirtir fariinta 'Processing' haddii aysan jirin wax khalad ah oo ka hor istaagay
