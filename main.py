...
def transcribe_via_assemblyai(audio_url, language_code):
    ...
    def _map_lang(code):
        ...
        if c in two_map: return two_map[c]
        if "_" in code or "-" in code: return code.replace("-", "_")
        return None
    mapped = _map_lang(language_code)
    data = {"audio_url": audio_url, "punctuate": True, "format_text": True}
    if mapped: data["language_code"] = mapped
    submit_url = ASSEMBLYAI_BASE_URL + "/transcript"
    resp = requests.post(submit_url, json=data, headers=headers, timeout=REQUEST_TIMEOUT_GEMINI)
    try:
        resp.raise_for_status()
    except Exception:
        raise RuntimeError(f"AssemblyAI submit failed ({resp.status_code}): {resp.text}")
    ...
    while True:
        transcription_result = requests.get(polling_endpoint, headers=headers, timeout=REQUEST_TIMEOUT_GEMINI)
        try:
            transcription_json = transcription_result.json()
        except:
            raise RuntimeError(f"AssemblyAI polling returned non-json ({transcription_result.status_code}): {transcription_result.text}")
        status = transcription_json.get('status')
        if status == 'completed':
            text = transcription_json.get('text')
            if text is None: raise RuntimeError("AssemblyAI returned 'completed' but with no text.")
            return text
        elif status == 'error':
            error_msg = transcription_json.get('error', 'Unknown AssemblyAI error')
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def on_result_mode_select(call):
        ...
        except: bot_obj.answer_callback_query(call.id,"❌ Error setting result mode",show_alert=True)
