...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename,file_url=None):
    ...
    try:
        if file_url is None: tf,file_url=telegram_file_info_and_url(bot_token,file_id)
        try:
...
def process_file_link(message,bot_obj,file_url):
    uid=str(message.from_user.id); chatid=str(message.chat.id)
    lang=get_stt_user_lang(uid)
    processing_msg=bot_obj.send_message(message.chat.id,"🔄 Processing link...",reply_to_message_id=message.message_id)
    processing_msg_id=processing_msg.message_id
    stop={"stop":False}
    animation_thread=threading.Thread(target=animate_processing_message,args=(bot_obj,message.chat.id,processing_msg_id,lambda:stop["stop"]))
    animation_thread.start()
    try:
        try:
            ...
            error_msg=str(e); logging.exception("Error during link transcription")
            if is_transcoding_like_error(error_msg):
                bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible or link is invalid. Please check the file and link.",reply_to_message_id=message.message_id)
            else:
                bot_obj.send_message(message.chat.id,f"Error during transcription: {error_msg}",reply_to_message_id=message.message_id)
            return
        corrected_text=normalize_text_offline(text)
    ...
    while True:
        try:
            ...
            if item:
                ...
                if file_url: process_file_link(message,bot_obj,file_url)
                else: process_media_file(message,bot_obj,bot_token,file_id,file_size,filename)
            else:
...
def handle_fileio_link(message,bot_obj):
    if not bot_obj: return
    update_user_activity(message.from_user.id)
    if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
        send_subscription_message(message.chat.id,bot_obj); return
...
def register_handlers(bot_obj,bot_token):
    ...
    def handle_help(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            text="Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video (up to 20MB) or a file.io link and I will transcribe it Need help? Contact: @lakigithub"
    ...
    def handle_text_messages(message):
        try:
            ...
            # Check for file.io link
            if re.match(r'https?:\/\/(?:www\.)?file\.io\/[a-zA-Z0-9]+',text):
                handle_fileio_link(message,bot_obj); return
