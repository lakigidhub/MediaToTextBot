...
from itsdangerous import URLSafeSerializer, SignatureExpired, BadSignature # Beddel: Waxaa laga beddelay URLSafeTimedSerializer
...
serializer = URLSafeSerializer(SECRET_KEY) # Beddel: Waxaa laga beddelay URLSafeTimedSerializer
...
def signed_upload_token(chat_id, lang_code):
    # Waxaan u beddelay 'serializer.dumps' kaas oo ka yimid URLSafeSerializer, ma laha wakhti uu ku dhamaanayo
    return serializer.dumps({"chat_id": chat_id, "lang": lang_code})

def unsign_upload_token(token, max_age_seconds=None): # Waxaan ka saarnay max_age_seconds, si uusan u dhamaan
    # Waxaan u beddelay 'serializer.loads' kaas oo ka yimid URLSafeSerializer, ma laha xad wakhti
    # Haddii aad si fudud u isticmaasho URLSafeSerializer, ma laha 'max_age' parameter.
    # Waana sababta aan u baahannay inaan ka saarno.
    try:
        return serializer.loads(token)
    except SignatureExpired:
        # Tani marna dhici mayso haddii la isticmaalayo URLSafeSerializer, laakiin waxaan u daayay BadSignature.
        # Haddii uu jiro parameter 'max_age' oo la isticmaalayo (sida aad markii hore u samaysay), SignatureExpired waa dhacaysaa.
        # Laakin maadaama aan hadda isticmaalayno URLSafeSerializer, waxaanu hubinaynaa oo kaliya BadSignature.
...
def upload_large_file(token):
    ...
    try:
        data = unsign_upload_token(token) # Beddel: Waxaa laga saaray max_age_seconds=3600
    except SignatureExpired:
        # SignatureExpired marnaba ma dhici doonto marka la isticmaalayo URLSafeSerializer
