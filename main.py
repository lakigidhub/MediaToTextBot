...
def transcribe_file_with_assemblyai(audio_url,language_code):
    # Halkan waxaan hadda u malaynaynaa in lagu soo diray AUDIO URL oo kaliya, si toos ah AssemblyAI u soo dejiso
    ...
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            ...
            # Xaalad kasta, waxaan u isticmaalaynaa URL-ka tooska ah ee faylka Telegram
...
def transcribe_via_selected_service(audio_url,lang_code):
    try:
        # Hadda waxaan diraynaa URL-ka tooska ah ee faylka si kasta
        text=transcribe_file_with_assemblyai(audio_url,lang_code)
...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename):
    ...
    # Hadda ma samaynayno wax soo dejin ah server-ka. Waxaan si toos ah u diraynaa URL-ka.
    try:
        file_url=telegram_file_info_and_url(bot_token,file_id)
        if not file_url: raise RuntimeError("Could not get file URL.")
        
        # Waxaan isticmaalaynaa habka URL-ka tooska ah ee AssemblyAI labada xaaladoodba.
        ...
        # Ma jirto tmpf_name oo la tirtiro hadda, sababtoo ah wax soo dejin ah ma samayno
...
def handle_media_common(message,bot_obj,bot_token):
    ...
    # Ma u baahnin inaan hubino cabirka faylka hadda, sababtoo ah had iyo jeer waxaan isticmaalaynaa habka URL-ka.
