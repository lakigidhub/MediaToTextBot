...
from pyrogram import Client,enums # Ku dar pyrogram

logging.basicConfig(level=logging.INFO,format='%(asctime)s - %(levelname)s - %(message)s')

env=os.environ

# Waxaan u kordhinay xadka si uu u gaaro xadka Pyrogram (illaa 2GB)
TELEGRAM_MAX_BYTES=int(env.get("TELEGRAM_MAX_BYTES",str(2048*1024*1024))) # Kordhi ilaa 2GB
...
# Ku dar API_ID iyo API_HASH
API_ID=env.get("API_ID","")
API_HASH=env.get("API_HASH","")
SESSION_NAME=env.get("PYRO_SESSION_NAME","my_bot_session")

if not BOT_TOKEN: logging.error("BOT_TOKEN is not set. Bot will not function.")
if not API_ID or not API_HASH: logging.warning("API_ID or API_HASH is not set. Pyrogram file handling will not work.")
...
# Initalize Pyrogram Client for large file downloads/uploads
pyrogram_client=None
if API_ID and API_HASH and BOT_TOKEN:
    try:
        pyrogram_client=Client(SESSION_NAME,api_id=int(API_ID),api_hash=API_HASH,bot_token=BOT_TOKEN,in_memory=True)
        pyrogram_client.start()
        logging.info("Pyrogram Client (User/Bot) started successfully.")
...
# Badalida shaqadaan si loo isticmaalo Pyrogram download
def telegram_file_info_and_url(bot_token,file_id):
    if pyrogram_client:
        # Pyrogram waxay ku shaqeyn kartaa gudaha thread-ka telebot haddii loo isticmaalo sida hoose
        # Si loo helo file path-ka/object-ga, waxaan isticmaaleynaa Telebot API-ga sidii hore
        url=f"https://api.telegram.org/bot{bot_token}/getFile?file_id={file_id}"
        resp=requests.get(url,timeout=REQUEST_TIMEOUT_TELEGRAM); resp.raise_for_status()
        file_path=resp.json().get("result",{}).get("file_path")
        
        # Waxaan soo celinaynaa object-ka Telebot
        return type('T',(),{'file_path':file_path})(), None # Pyrogram download ma isticmaasho URL-kan

    # Haddii Pyrogram uusan diyaar ahayn, ku laabo habka hore (oo leh xad)
    url=f"https://api.telegram.org/bot{bot_token}/getFile?file_id={file_id}"
    resp=requests.get(url,timeout=REQUEST_TIMEOUT_TELEGRAM); resp.raise_for_status()
    file_path=resp.json().get("result",{}).get("file_path")
...
def transcribe_via_selected_service(input_path,lang_code):
    try:
        text,used_service=transcribe_file_with_assemblyai(input_path,lang_code)
...
def process_media_file(message,bot_obj,bot_token,file_id,file_size,filename):
    ...
    tmpf_name=None
    try:
        tmpf_suffix="."+ (safe_extension_from_filename(filename) or "tmp")
        tmpf=tempfile.NamedTemporaryFile(delete=False,suffix=tmpf_suffix)
        tmpf_name=tmpf.name
        tmpf.close()
        
        # Download the file using Pyrogram or requests
        if pyrogram_client:
            # Ku xir Telebot object-ga Pyrogram object si loo soo dejiyo
            file_ref=None
            if message.voice: file_ref=message.voice
            elif message.audio: file_ref=message.audio
            elif message.video: file_ref=message.video
            elif message.document: file_ref=message.document

            if file_ref:
                # Pyrogram download. Wuxuu maamulaa faylalka waaweyn si toos ah.
                # Waa inaan ku sameynaa meesha uu ku yaalo
                from pyrogram.file_id import FileId
                file_id_obj=FileId.decode(file_id)
                pyrogram_client.download(file_id_obj,file_name=tmpf_name)
            else:
                raise RuntimeError("File reference not found in message.")
        else:
            # Habka rasmiga ah ee Telebot (leh xad)
...
def handle_media_common(message,bot_obj,bot_token):
    ...
    # Xad-gudubka faylalka ka dib markii Pyrogram la isticmaalo. Tani waa check gudaha ah.
    if file_size and file_size>TELEGRAM_MAX_BYTES:
        max_display_gb=TELEGRAM_MAX_BYTES//(1024*1024*1024)
        if max_display_gb==0: max_display_gb=1 # display 1GB if max_bytes is between 1GB and 2GB
        bot_obj.send_message(message.chat.id,f"Just Send me a file less than {max_display_gb}GB 😎",reply_to_message_id=message.message_id); return
    with memory_lock:
...
def register_handlers(bot_obj,bot_token):
    ...
    def start_select_lang_callback(call):
        try:
            ...
            if pyrogram_client: welcome_text+="\n\n**✅ Large file support is active!**"
            else: welcome_text+="\n\n**⚠️ Limited to 50MB (Enable Pyrogram for 2GB limit)**"
    ...
    def handle_help(message):
        try:
            ...
            if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
                send_subscription_message(message.chat.id,bot_obj); return
            max_file_size_gb=TELEGRAM_MAX_BYTES//(1024*1024*1024)
            if max_file_size_gb==0: max_file_size_gb=1
            max_size_text=f"up to {max_file_size_gb}GB" if pyrogram_client else "up to 50MB"
            text=f"Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video ({max_size_text}) and I will transcribe it Need help? Contact: @lakigithub"
            if pyrogram_client: text="\n\n✅ Large file support is active! "+text
