...
AUDIO_OUTPUT_FORMAT=env.get("AUDIO_OUTPUT_FORMAT","mp3").lower()
...
def convert_to_output_format(input_path,output_path):
    run_ffmpeg(["-y","-i",input_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),output_path])

def get_audio_duration(file_path):
    try:
        p=subprocess.run([FFMPEG_BINARY,"-i",file_path,"-f","null","-"],stderr=subprocess.PIPE,check=True,timeout=10)
        output=p.stderr.decode("utf-8"); match=re.search(r"Duration: (\d{2}):(\d{2}):(\d{2}\.\d{2})",output)
        if match:
...
def prepend_silence(original_audio,output_audio,silence_sec):
    tmp_dir=os.path.dirname(output_audio) or tempfile.gettempdir()
    silence_file=os.path.join(tmp_dir,f"silence_{int(time.time()*1000)}.{AUDIO_OUTPUT_FORMAT}")
    ...
    run_ffmpeg(["-y","-i",silence_file,"-i",original_audio,"-filter_complex","[0:0][1:0]concat=n=2:v=0:a=1[out]","-map","[out]",output_audio])
    try: os.remove(silence_file)
    except: pass

def split_to_chunks_with_overlap(audio_path,out_dir,chunk_duration_sec,overlap_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    ...
    if total_dur==0:
        logging.warning("Could not determine audio duration, treating as single chunk.")
        out_name=os.path.join(out_dir,f"chunk000.{AUDIO_OUTPUT_FORMAT}")
        convert_to_output_format(audio_path,out_name)
        return [out_name]
    ...
    for i in range(num):
        ...
        out_name=os.path.join(out_dir,f"chunk{i:03d}.{AUDIO_OUTPUT_FORMAT}")
...
def create_prepended_chunk(chunk_path,silence_sec):
    tmp=tempfile.NamedTemporaryFile(delete=False,suffix=f".{AUDIO_OUTPUT_FORMAT}"); tmp.close()
    try:
        prepend_silence(chunk_path,tmp.name,silence_sec); return tmp.name
    except:
...
def transcribe_file_with_speech_recognition(input_file_path,language_code):
    ...
    try:
        base_audio=os.path.join(tmpdir,f"converted.{AUDIO_OUTPUT_FORMAT}")
        try: convert_to_output_format(input_file_path,base_audio)
        except Exception as e: raise RuntimeError(f"Conversion to {AUDIO_OUTPUT_FORMAT} failed: "+str(e))
        chunk_files=split_to_chunks_with_overlap(base_audio,tmpdir,CHUNK_DURATION_SEC,CHUNK_OVERLAP_SEC)
