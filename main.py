...
AUDIO_SAMPLE_RATE=int(env.get("AUDIO_SAMPLE_RATE","8010"))
...
SPEECH_RECOGNITION_FORMAT=env.get("SPEECH_RECOGNITION_FORMAT","pcm").lower() # Doorsoome Cusub
...
def convert_to_preferred_format(input_wav_path,output_preferred_path,format_ext):
    if format_ext=="wav":
        os.rename(input_wav_path,output_preferred_path)
        return
    run_ffmpeg(["-y","-i",input_wav_path,"-f",format_ext,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),output_preferred_path])
...
def recognize_chunk_file(recognizer,file_path,language,preferred_format=SPEECH_RECOGNITION_FORMAT):
    last_exc=None; prep=None; preferred_file_path=None
    try:
        if preferred_format!="wav":
            tmp_dir=os.path.dirname(file_path) or tempfile.gettempdir()
            preferred_file_path=os.path.join(tmp_dir,f"{os.path.basename(file_path).split('.',1)[0]}.{preferred_format}")
            convert_to_preferred_format(file_path,preferred_file_path,preferred_format)
            use_path=preferred_file_path
        else:
            use_path=file_path

        # Tallaabada 1: Isku hagaajinta buuqa Ambient-ka (wuxuu u baahan yahay .wav file)
        with sr.AudioFile(file_path) as source:
            try: recognizer.adjust_for_ambient_noise(source,duration=AMBIENT_CALIB_SEC)
            except: pass
            audio_data=recognizer.record(source)

        # Tallaabada 2: Haddii ay jirto silence, dib u qor audio-ga oo ku dar
        if PREPEND_SILENCE_SEC>0:
            # U isticmaal .wav oo la hagaajiyay ee uu bixiyo library-ga ka dib ambient noise adjustment
            # Haddii aad si toos ah u isticmaasho use_path (preferred_file_path), waxaad ka boodi kartaa ambient adjustment-ka kore
            # Haddaba, waxaan u beddelnaa qaabka uu u shaqeynayo prep
            prep_tmp=tempfile.NamedTemporaryFile(delete=False,suffix=f".{preferred_format}"); prep_tmp.close()
            with sr.AudioFile(file_path) as source: # Waa inaan ku helnaa AudioData oo la hagaajiyay
                audio_data_for_prep=recognizer.record(source)
                with open(prep_tmp.name,"wb") as f:
                    # Tani ma shaqayn doonto si toos ah oo la hubo in lagu qoro format-ka cusub sababtoo ah AudioData.write_*-ku
                    # wuxuu u baahan yahay module la mid ah audioofl
                    # Halkan waxaan si toos ah u qorayaa .wav mar labaad si aan u xaqiijiyo in uu shaqeynayo
                    # Waxaa fiican in la isticmaalo chunking-ka hore iyo prepend_silence_to_wav
                    pass # Ka bood pre-pend-ka hadda, si looga fogaado khalad ku yimaada maktabadda speech_recognition

            # Si loo fuliyo shuruudda pre-pend, waxaan u isticmaalaynaa isbeddelkii hore, laakiin waxay u baahan tahay inaan u beddelno format-ka la rabo ka dib.
            if preferred_file_path:
                prep=create_prepended_chunk(file_path,PREPEND_SILENCE_SEC) # Weli wuxuu u shaqeynayaa .wav
                # Hadda waxaan u beddelaynaa foomatka la rabo si aan u isticmaalno
                os.remove(preferred_file_path)
                convert_to_preferred_format(prep,preferred_file_path,preferred_format)
                use_path=preferred_file_path
                # Hadda waxaan u baahanahay inaan dib u akhrino use_path si aan ugu dirno API-ga Google
                with sr.AudioFile(use_path) as source: # Isticmaal sr.AudioFile xitaa haddii aysan ahayn .wav, maxaa yeelay waxay taageeri kartaa flac, iwm.
                    audio=recognizer.record(source)
            else: # Haddii format-ku yahay wav
                prep=create_prepended_chunk(file_path,PREPEND_SILENCE_SEC)
                use_path=prep
                with sr.AudioFile(use_path) as source:
                    audio=recognizer.record(source)
        else: # Haddii uusan jirin silence pre-pend
            # Hadda, ka dib ambient noise adjustment, waxaan isticmaalaynaa use_path si aan ugu dirno API-ga Google.
            # Haddii use_path uu yahay preferred_file_path, waa inaan helnaa audio data oo ka yimid
            # faylka preferred_file_path.
            if preferred_file_path:
                with sr.AudioFile(preferred_file_path) as source:
                    audio=recognizer.record(source)
            else: # Waa wav asalka ah
                audio=audio_data # Wuxuu ka yimid tallaabada 1aad

        # Tallaabada 3: Dirida API-ga Google-ka
        text=recognizer.recognize_google(audio,language=language) if language else recognizer.recognize_google(audio)

        if prep:
            try: os.remove(prep)
            except: pass
        if preferred_file_path and preferred_file_path!=file_path:
            try: os.remove(preferred_file_path)
            except: pass
        return text
...
def transcribe_file_with_speech_recognition(input_file_path,language_code):
    ...
    try:
        ...
        def t(c): return recognize_chunk_file(sr.Recognizer(),c,language_code,SPEECH_RECOGNITION_FORMAT)
