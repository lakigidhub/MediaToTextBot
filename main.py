...
def transcribe_with_assemblyai(input_file_path,language_code):
    ...
    config={"audio_url":audio_url,"language_code":language_code,"auto_chapters":False,"auto_highlights":False,"word_boost":False,"punctuate":True,"format_text":True}
    if language_code!="en": config["language_detection"]=False
    if language_code: config["language_code"]=language_code
...
def transcribe_via_selected_service(input_path,lang_code):
    if ASSEMBLYAI_API_KEY:
        try:
            ...
            logging.warning("AssemblyAI failed, falling back to speech_recognition: "+str(e))
    try:
