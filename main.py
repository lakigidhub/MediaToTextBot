...
def update_user_setting(user_id,key,value,upsert=False):
    update={"$set":{key:value}}
    if upsert: update["$setOnInsert"]={"first_seen":datetime.now()}
    users_collection.update_one({"user_id":norm_user_id(user_id)},update,upsert=upsert)

def get_user_setting(user_id,key,default):
    ud=users_collection.find_one({"user_id":norm_user_id(user_id)})
    return ud.get(key,default) if ud else default
...
def get_stt_user_lang(user_id):
    return get_user_setting(user_id,"stt_language","en")

def set_stt_user_lang(user_id,lang_code):
    update_user_setting(user_id,"stt_language",lang_code,upsert=True)

def get_user_send_mode(user_id):
    return get_user_setting(user_id,"stt_send_mode","file")

def set_user_send_mode(user_id,mode):
    if mode not in ("file","split"): mode="file"
    update_user_setting(user_id,"stt_send_mode",mode,upsert=True)
...
def send_transcribed_text(bot_obj,chat_id,reply_to_message_id,corrected_text,uid_key):
    ...
    if len(corrected_text)>4000:
        ...
        sent=bot_obj.send_message(chat_id,corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=reply_to_message_id)
    
    if sent:
        try: 
            attach_action_buttons(bot_obj,chat_id,sent.message_id,corrected_text)
    ...
    try:
        ...
        try:
            ...
            if "ffmpeg" in error_msg.lower() and not FFMPEG_BINARY:
                err="⚠️ Server error: ffmpeg not found. Notify the admin."
            elif any(ch in error_msg.lower() for ch in ["transcoding failed","file does not appear to contain audio","text/html","html document","unsupported media type","could not decode"]):
                err="⚠️ Transcription error: file is not audible. Please send a different file."
            else:
                err=f"Error during transcription: {error_msg}"
            bot_obj.send_message(message.chat.id,err,reply_to_message_id=message.message_id); return

        corrected_text=normalize_text_offline(text)
        send_transcribed_text(bot_obj,message.chat.id,message.message_id,corrected_text,str(message.chat.id))
...
def handle_media_common(message,bot_obj,bot_token):
    ...
    file_info=None
    if message.voice:
        file_info={'file_id':message.voice.file_id,'file_size':message.voice.file_size,'filename':"voice.ogg"}
    elif message.audio:
        file_info={'file_id':message.audio.file_id,'file_size':message.audio.file_size,'filename':getattr(message.audio,"file_name","audio")}
    elif message.video:
        file_info={'file_id':message.video.file_id,'file_size':message.video.file_size,'filename':getattr(message.video,"file_name","video.mp4")}
        ...
        if (mime and ("audio" in mime or "video" in mime)) or ext in ALLOWED_EXTENSIONS:
            file_info={'file_id':message.document.file_id,'file_size':message.document.file_size,'filename':filename}
        else:
            bot_obj.send_message(message.chat.id,"Sorry, I can only transcribe audio or video files."); return

    if file_info and file_info['file_size']>TELEGRAM_MAX_BYTES:
    ...
    if file_info:
        with memory_lock:
            ...
            PENDING_QUEUE.append((message,bot_obj,bot_token,file_info['file_id'],file_info['file_size'],file_info['filename']))

def ask_gemini(text,instruction,timeout=REQUEST_TIMEOUT_GEMINI):
    if not GEMINI_API_KEYS: raise RuntimeError("GEMINI_API_KEYS not set or is empty")
    last_exception=None
        ...
        try:
            ...
            if "candidates" in result and isinstance(result["candidates"],list) and len(result["candidates"])>0:
                try: return result['candidates'][0]['content']['parts'][0]['text']
                    ...
                    logging.warning(f"Gemini API key {api_key[:4]}... returned a result without expected text format: {json.dumps(result['candidates'][0])}")
                    return json.dumps(result['candidates'][0])
            else:
                raise RuntimeError(f"Gemini API key {api_key[:4]}... returned an invalid response structure.")
        except Exception as e:
            logging.warning(f"Gemini API key {api_key[:4]}... failed with exception: {str(e)}. Trying next key if available.")
            last_exception=e
            continue
...
def handle_ai_action(call,instruction,callback_key,err_msg,success_func):
    ...
    else: bot.answer_callback_query(call.id,"Invalid request",show_alert=True); return
    try: chat_id_val=int(chat_id_part); msg_id=int(msg_id_part)
    except: bot.answer_callback_query(call.id,"Invalid message id",show_alert=True); return
    usage_key=f"{chat_id_val}|{msg_id}|{callback_key}"; usage=action_usage.get(usage_key,0)
    if usage>=1: bot.answer_callback_query(call.id,f"{err_msg} unavailable (maybe expired)",show_alert=True); return
    action_usage[usage_key]=usage+1
    uid_key=str(chat_id_val); stored=user_transcriptions.get(uid_key,{}).get(msg_id)
    if not stored: bot.answer_callback_query(call.id,f"{err_msg} unavailable (maybe expired)",show_alert=True); return
    bot.answer_callback_query(call.id,"Generating...")
    status_msg=bot.send_message(call.message.chat.id,"🔄 Processing...",reply_to_message_id=call.message.message_id)
    stop={"stop":False}
    animation_thread=threading.Thread(target=animate_processing_message,args=(bot,call.message.chat.id,status_msg.message_id,lambda:stop["stop"]))
    animation_thread.start()
    try:
        lang=get_stt_user_lang(str(chat_id_val)) or "en"
        full_instruction=instruction.format(lang=lang)
        result=ask_gemini(stored,full_instruction)
    except Exception as e:
        result=""; logging.exception(f"Error during AI action ({callback_key})")
    finally:
        stop["stop"]=True; animation_thread.join()
    ...
    if not result:
        try: bot.send_message(call.message.chat.id,f"No {err_msg} returned.",reply_to_message_id=call.message.message_id)
        except: pass
    else:
        success_func(call.message.chat.id,call.message.message_id,result,status_msg.message_id)

def register_handlers(bot_obj,bot_token):
    if not bot_obj: return

    def check_sub_and_update(message):
        update_user_activity(message.from_user.id)
        if message.chat.type=='private' and not check_subscription(message.from_user.id,bot_obj):
            send_subscription_message(message.chat.id,bot_obj); return False
        return True
    
    @bot_obj.message_handler(commands=['start'])
    def start_handler(message):
        if not check_sub_and_update(message): return
    ...
    def summarize_success(chat_id,reply_to_message_id,summary,deleted_msg_id):
        try: bot.send_message(chat_id,f"{summary}",reply_to_message_id=reply_to_message_id)
        except: pass

    @bot_obj.callback_query_handler(lambda c:c.data and c.data.startswith("get_key_points|"))
    def get_key_points_callback(call):
        instruction="What is this report and what is it about? Please summarize them for me into (lang={lang}) without adding any introductions, notes, or extra phrases."
        handle_ai_action(call,instruction,"get_key_points","Get Summarize",summarize_success)

    def cleanup_success(chat_id,reply_to_message_id,cleaned,deleted_msg_id):
        uid_key=str(chat_id)
        send_transcribed_text(bot,chat_id,reply_to_message_id,cleaned,uid_key)
    ...
    def clean_up_callback(call):
        ...
        handle_ai_action(call,instruction,"clean_up","Clean up",cleanup_success)
