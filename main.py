...
SPEECH_RECOGNITION_FORMAT=env.get("SPEECH_RECOGNITION_FORMAT","pcm").lower()
...
def convert_to_wav(input_path,output_wav_path):
    # Tani waxay had iyo jeer u beddeshaa WAV si loo fududeeyo chunking-ga maktabadda speech_recognition
...
def convert_to_preferred_format(input_wav_path,output_preferred_path,format_ext):
    if format_ext=="wav":
        ...
        return
    # Isticmaal ffmpeg si aad ugu beddesho foomatka la rabo ee loo dirayo API-ga
...
def recognize_chunk_file(recognizer,file_path,language,preferred_format=SPEECH_RECOGNITION_FORMAT):
    ...
    try:
        # Tallaabada 1: Ambient Noise Adjustment (Wuxuu u baahan yahay fayl la aqbali karo, sida .wav-ka uu yimid)
        with sr.AudioFile(file_path) as source:
            try: recognizer.adjust_for_ambient_noise(source,duration=AMBIENT_CALIB_SEC)
            except: pass
            audio_data=recognizer.record(source)

        # Tallaabada 2: Prepend Silence (Haddii loo baahdo) iyo Beddelidda Foomatka
        use_path=file_path
        if PREPEND_SILENCE_SEC>0:
            # Ku dar silence (weli wuxuu u shaqeynayaa .wav)
            prep=create_prepended_chunk(file_path,PREPEND_SILENCE_SEC)
            use_path=prep
            # Dib u qor audio data si aad u hesho audio-ga cusub ee leh silence
            with sr.AudioFile(use_path) as source:
                audio_data=recognizer.record(source)

        # Tallaabada 3: Beddel foomatka loo dirayo API-ga (Haddii uusan ahayn WAV)
        if preferred_format!="wav":
            ...
            # Kaydi audio-data ka oo ah .wav ka dib ambient/silence si aad u beddesho
            temp_wav_for_convert=tempfile.NamedTemporaryFile(delete=False,suffix=".wav"); temp_wav_for_convert.close()
            with open(temp_wav_for_convert.name,"wb") as f:
                # Si loo xalliyo, waxaan u baahanahay inaan kaydinno audio_data-ga ka dib marka la hagaajiyo/kalo daro.
                # Markii aan isticmaalnay prepended_chunk, waxaan haynaa faylka. Haddi aan la isticmaalin, waxaan ku qoreynaa data-ha la hagaajiyay.
                if use_path==file_path: # Haddii aan la isticmaalin prep, waxaan u qoreynaa temp file si aan u beddelno
                     with open(file_path,'rb') as f_in: f.write(f_in.read()) # Tani ma hubiso ambient adjustment
                else: # Haddii prep la isticmaalay
                    with open(use_path,'rb') as f_in: f.write(f_in.read())
            
            # Hadda u beddel foomatka la rabo
            convert_to_preferred_format(temp_wav_for_convert.name,preferred_file_path,preferred_format)
            use_path=preferred_file_path
            
            # Akhriso foomatka la rabo
            with sr.AudioFile(use_path) as source:
                audio=recognizer.record(source)
            os.remove(temp_wav_for_convert.name)
        else:
            # Haddii format-ku yahay wav, waxaan isticmaaleynaa audio_data (oo laga yaabo in laga helo faylka prepended)
            audio=audio_data

        # Tallaabada 4: Dirida API-ga Google-ka
